...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {/* ── Mobile layout (< lg) ── */}
      <div className="lg:hidden min-h-screen flex flex-col w-full bg-background/50 relative overflow-hidden">
        <main className="flex-1 overflow-y-auto pb-24 px-6 pt-8 scrollbar-hide">
          {children}
        </main>

        <nav className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-t border-border z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
